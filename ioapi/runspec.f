        ...
        !!***********************************************************************
        !! Version "$Id: runspec.f 196 2015-06-18 13:50:15Z coats $"
        ...
        IF ( USEENV ) THEN
            ...
            SDATE = ENVINT( 'SDATE', 'Starting date (YYYYDDD)',
     &                       SDATE1, ISTAT )
            IF ( ISTAT .GT. 0 ) THEN
                EFLAG = .TRUE.
            ...
            STIME = ENVINT( 'STIME', 'Starting time (HHMMSS)',
     &                       STIME1, ISTAT )
            IF ( ISTAT .GT. 0 ) THEN
                EFLAG = .TRUE.
            ...
            EDATE = ENVINT( 'EDATE', 'Ending date (YYYYDDD)',
     &                       EDATE1, ISTAT )
            IF ( ISTAT .GT. 0 ) THEN
                EFLAG = .TRUE.
            ...
            ETIME = ENVINT( 'ETIME', 'Ending time (HHMMSS)',
     &                       ETIME1, ISTAT )
            IF ( ISTAT .GT. 0 ) THEN
                EFLAG = .TRUE.
            ...
            TSTEP = ENVINT( 'TSTEP', 'Time step (H*MMSS)',
     &                       TSTEP1, ISTAT )
            IF ( ISTAT .GT. 0 ) THEN
                EFLAG = .TRUE.
        ...
        ELSE
            ...
            SDATE = GETNUM( SDATE1, 9999999, JDATE1,
     &                 'Enter STARTING DATE (YYYYDDD)' )

            STIME = GETNUM( 0,   9999999, STIME1,
     &                 'Enter STARTING TIME  (H*MMSS)' )

            TSTEP = GETNUM( TSTEP1, 999999999, TSTEP1,
     &                 'Enter     TIME STEP  (H*MMSS)' )

            EDATE = GETNUM( SDATE, 9999999, EDATE1,
     &                  'Enter  ENDING DATE (YYYYDDD)' )

            ETIME = GETNUM( 0, 9999999, ETIME1,
     &                 'Enter   ENDING TIME  (H*MMSS)' )
     ...
     &          'not compatible with "', TRIM( FNAME ) , '"'
            CALL M3MESG( MESG )
        END IF
