MODULE MODPDATA
    ...
    !!***********************************************************************
    !! Version "$Id: modpdata.F90 273 2015-12-03 18:05:34Z coats $"
...
!$OMP   SINGLE
        ...
        CALL M3MESG( MESG )
        CALL M3MESG( &
'$Id: modpdata.F90 273 2015-12-03 18:05:34Z coats $' )

        IF ( .NOT.ENVLIST( 'NPCOL_NPROW', 'Processor decomposition: npcol x nprow', 2, N, PLIST ) ) THEN
            MESG = 'Bad environment-list "NPCOL_NPROW"'
            CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
        ELSE IF ( N .NE. 2 ) THEN
            MESG = 'Bad #(entries) for environment-list "NPCOL_NPROW"'
            CALL M3EXIT( PNAME, 0, 0, MESG, 2 )
        ELSE IF ( PLIST(1)*PLIST(2) .LE. 0 ) THEN
